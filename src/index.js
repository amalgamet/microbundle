...
import { generateBundle } from './lib/generate-bundle';
...
export default async function microbundle(inputOptions) {
	...
	if (entries.length > 1) {
		entries.forEach(entry => {
			const isIndexMatch = entry.match(
				/([\\/])index(\.(umd|cjs|es|m))?\.(mjs|[tj]sx?)$/,
			);

			const mainNoExtension = isIndexMatch ? output : entry;
			mappedEntries[
				basename(mainNoExtension).replace(
					/(\.(umd|cjs|es|m))?\.(mjs|[tj]sx?)$/,
					'',
				)
			] = entry;
		...
		mappedEntries[options.pkg.name] = entries[0];
	}

	let nameCache = {};
	let minifyOptions = {};
	function loadNameCache(getNameCachePath) {
		try {
	...
	// TODO figure out how to handle multiple entries with umd & iife
	// or should we fail hard?
	const bundles = await generateBundle({
		...options,
	...
	const sizeInfos = await Promise.all(
		bundles
			.map(({ output }) => {
				return output.map(({ code, fileName }) => {
					if (!code) {
	...
	const targetDir = relative(cwd, dirname(output)) || '.';
	const banner = blue(`Build "${options.name}" to ${targetDir}:`);
	return `${banner}\n${sizeInfos.filter(Boolean).join('\n')}`;
}
...
async function getInput({ entries, cwd, source, module }) {
	let input = entries ? [].concat(entries) : [];

	if (!input.length) {
		if (source) {
			input = [].concat(source).map(file => resolve(cwd, file));
		}

		if (!input.length) {
			const fileName = await jsOrTs(cwd, 'src/index');
			input = (await isFile(fileName)) ? [fileName] : [];
		}

		if (!input.length) {
			const fileName = await jsOrTs(cwd, 'index');
			input = (await isFile(fileName)) ? [fileName] : [];
		}
	...
	return input.map(file => glob(file)).flat();
}
